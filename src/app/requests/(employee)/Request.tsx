...
import { Eye, Wrench, X, Play, Check, RotateCcw, Star, ClipboardCheck } from "lucide-react"
...
const RequestRow: React.FC<RequestRowProps> = ({ request }) => {
  ...
  const role = user?.role === ROLE.MANAGER ? "manager" : "employee"

  const handleChangeState = async (nextState: REQUESTS_STATE) => {
    await changeState({ requestId: request.id, state: nextState, role })
  }
    ...
    switch (state) {
      ...
      case REQUESTS_STATE.FINISHED: {
        ...
        if (role === "manager") {
          actions.push(
            {
              icon: <ClipboardCheck className="text-purple-500 group-hover:text-white" />,
              title: "Review",
              action: () => handleChangeState(REQUESTS_STATE.REVIEWED),
              className: "hover:bg-purple-500",
            },
            {
              icon: <Star className="text-amber-500 group-hover:text-white" />,
              title: "Succeed",
